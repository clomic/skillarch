...
|crypt-dec|Decrypt (AES-256-CBC) the file (a password will be required)|
|crypt-enc|Encrypt (AES-256-CBC) the file with an auto generated password which is copied to Clipboard|
|da|`cd /DATA`|
...
|dhashcat|dizcza/docker-hashcat \| CPU/GPU hash cracking|
...
|dl|`cd ~/Downloads`|
|dmsfconsole|metasploitframework \| Metasploit|
|dmsfvenom|metasploitframework \| Metasploit|
...
|ew|`cd ~/.exegol/workspaces/<container>`|
|exsta|`exegol start`|
|exsto|`exegol stop`|
...
|ip6d|Disable IPv6 via sysctl|
|ip6e|Enable IPv6 via sysctl|
...
|kdo|Caido Proxy (`caido --no-sandbox &> /dev/null &`)|
...
|nmap-i|Nmap full TCP and UDP ports|
...
|skau|Interactive upstream-merge for forks (commit local → fetch → preview → merge → push → reapply, all Y/n-prompted)|
...
|srsta|Start SysReptor|
|srsto|Stop SysReptor|
...
|xh|xxd <last_arg> \| head <args_but_last>  (`xh -n -20 file` == `xxd file \| head -n -20 file`)|
